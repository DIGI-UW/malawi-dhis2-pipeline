...
while [[ $# -gt 0 ]]; do
    case $1 in
        --debug|-d)
            ...
            ;;
        *)
            # Keep other arguments for the main function
            break
            ;;
    esac
done
...
function generate_dummy_certificates() {
    log info "Generating dummy certificates with certbot..."
    log debug "Domain: ${DOMAIN_NAME}, Email: ${RENEWAL_EMAIL}, Args: ${DOMAIN_ARGS[*]}"
    ...
    if [[ -z "${RENEWAL_EMAIL}" ]]; then
        log error "RENEWAL_EMAIL is not set"
        exit 1
    fi
    
    # Check if port 80 is available (required for standalone mode)
    if netstat -tuln 2>/dev/null | grep -q ":80 "; then
        log warn "Port 80 appears to be in use. This may cause certbot standalone mode to fail."
    fi
    
    # Run certbot with better error handling
    local certbot_cmd="docker run --rm \
        --network host \
        --name letsencrypt \
    ...
    if [[ "${DEBUG:-0}" == "1" ]]; then
        log debug "Running command: $certbot_cmd"
        if ! eval "$certbot_cmd"; then
            log error "Certbot command failed. Check the output above for details."
            exit 1
        fi
    else
        # Capture stderr for better error reporting
        local error_output
        if ! error_output=$(eval "$certbot_cmd" 2>&1); then
            log error "Failed to create certificate network"
            log error "Certbot error output: $error_output"
            exit 1
        fi
    fi
