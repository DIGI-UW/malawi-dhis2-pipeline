...
docker::deploy_config_importer() {
    ...
    # Check if configs already exist before deploying
    if config::configs_exist "$CONFIG_COMPOSE_PATH" "$CONFIG_LABEL"; then
        log info "Configs for $CONFIG_LABEL already exist, skipping config importer deployment"
        return 0
    fi
