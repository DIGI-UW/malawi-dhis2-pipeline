...
config::set_config_digests() {
    ...
    log info "compose_folder_path: ${compose_folder_path}"
...
# Checks if configs from the docker compose file already exist in Docker Swarm
#
# Requirements:
# - All configs must have a file and name property
# - The name property must end in -${DIGEST_VAR_NAME:?err} (eg. name: my-file-${MY_FILE_DIGEST:?err})
...
# Arguments:
# - $1 : docker compose directory path (eg. /home/user/project/docker-compose.yml)
# - $2 : config label (eg. openfn-workflow)
#
# Returns:
# - 0 if all configs exist, 1 if any are missing
#
config::configs_exist() {
    local -r DOCKER_COMPOSE_PATH="${1:?$(missing_param "configs_exist" "DOCKER_COMPOSE_PATH")}"
    local -r CONFIG_LABEL="${2:?$(missing_param "configs_exist" "CONFIG_LABEL")}"

    # Check if yq is available
    if ! command -v yq >/dev/null 2>&1; then
        log error "yq is required but not installed. Cannot check config existence."
        return 1
    fi
    ...
    for name in "${names[@]}"; do
        # Skip if this is a literal 'null' value
        if [[ "$name" == "null" ]]; then
            continue
        fi

        # Extract the base name (remove environment variable placeholders)
        local base_name=$(echo "${name}" | sed 's/-\${.*//g')
        
        # Check if any config with this base name exists
        local existing_configs=$(docker config ls -f "label=name=${CONFIG_LABEL}" -f "name=${base_name}" --format "{{.Name}}" 2>/dev/null)
        
        if [[ -z "$existing_configs" ]]; then
            log debug "Config with base name '${base_name}' does not exist"
            return 1
        fi
        
        log debug "Found existing config(s) for '${base_name}': ${existing_configs}"
    done

    log debug "All configs already exist in Docker Swarm"
    return 0
}
