 ...
 * This object is used as a fallback if the adaptor-provided configuration is unavailable.
 * Ideally, configurations should be managed within the adaptor's `fileTypeConfig.js`.
 */
const DEFAULT_FILE_TYPE_CONFIGS = {
  pepfar_tx_curr_csv: {
    fileType: 'csv',
  ...
  pepfar_tx_mmd_csv: {
    ...
    multiValueMapping: {
      strategy: 'split_by_column',
      categoryCode: 'MMD_DURATION',
      mappings: {
        mmd_lt3: 'MMD_LT3',
        mmd_3to5: 'MMD_3TO5',
        mmd_ge6: 'MMD_GE6'
...
function loadFileTypeConfigsSafe() {
  try {
    // Attempt to load from global scope (adaptor provided)
    if (typeof loadFileTypeConfigs === 'function') {
      const configs = loadFileTypeConfigs();
      if (configs && Object.keys(configs).length > 0) {
        console.log('✅ Using configuration from DHIS2 adaptor');
        return deepClone(configs);
      }
    }
  } catch (error) {
    console.log(`⚠️  Could not load adaptor configuration: ${error.message}`);
  }
  
  console.log('⚠️  Using default internal configuration (Fallback)');
  return deepClone(DEFAULT_FILE_TYPE_CONFIGS);
}
...
execute(fn(async state => {
  ...
  const fileTypeConfigs = loadFileTypeConfigsSafe();
