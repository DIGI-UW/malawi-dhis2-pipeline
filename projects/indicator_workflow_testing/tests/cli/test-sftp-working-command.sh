...
docker run --rm -i --network openfn_public -v "$(pwd)/../../../openfn-workflows/workflows/sftp-test/state:/e2e" openfn-cli-test:latest /bin/sh <<'EOF'
# Create proper OpenFN project structure
mkdir -p /tmp/myproject/workflows/test-workflow

# Create openfn.json configuration
cat > /tmp/myproject/openfn.json << 'INNER_EOF'
{
...
cat > /tmp/myproject/workflows/test-workflow/test-workflow.json << 'INNER_EOF'
{
  "id": "test-workflow",
  "steps": [
    {
...
cat /e2e/sftp-test-state.json

echo ''
echo 'Running workflow with -s flag (NOT -i which is autoinstall)...'
openfn myproject test-workflow -s /e2e/sftp-test-state.json -o output.json 2>&1
...
echo "  • Using unified state file from workflow directory" 